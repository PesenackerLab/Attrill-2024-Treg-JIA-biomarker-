...
#Load rcc and Positive control normalisation----

rccdata <- load_rcc(here('data','raw_rcc'),
                    here('data','annotations.csv'),
                    'file_name',
...
#Added extra colum for positive control normalisation flags
#TRUE = outside range
...
write_csv(poscontrolflag, here('output','poscontrolflag.csv'))


#Write Csv----
csvdata <- write_csv(rccdata[["nacho"]], 
                     here('output', 'normdata.csv'))
...
write_csv(posnormdata, here('output','posnormdata.csv'))
...
#Add annotations----
annot <- read_csv(here('data','annotations.csv'))
total_norm <- total_norm %>%
  mutate (annot)
...
full_norm <- write_csv(full_norm, here('output','full_norm.csv'))
...
write_csv(no_flag, here('output','no_flag.csv'))


#Log2 transformation----
#Convert 0 to 1
log2_no_flag <- read_csv(here('output','no_flag.csv'))
...
write_csv(log2_noflag_data, here('output', 'log2_noflag_data.csv'))
